...
In this hands-on exercise, I learn how to compute Global and Local Measure of Spatial Autocorrelation (GLSA) by using **spdep** package. By the end to this hands-on exercise, you will be able to:
...
Since I\>0, the observations are clustered and tend to be similar.
...
Instead of using Base Graph to plot the values, ggplot2 package is used below:
...
Question: What statistical conclusion can you draw from the output above? Since the c \<1, observations tend to be clustered and similar.
...
In the top right corner of the map, there are 5 cities with higher GDP per capita and their neighbours with high GDP per capita too (in red), those (in light blue) that are right outside those 5 cities are outliers with low GDP per capita among neighbours with high GDP per capita).

On the bottom left corner, there is a city highlighted in dark blue as low-low, meaning it has low GDP per capita along with its neighbours with low GDP per capita.
...
Question: What statistical observation can you draw from the Gi map above? Overall, most cities in China are cold spot areas with some cities on the right side of the map being hot spot areas.
