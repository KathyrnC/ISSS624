...
-   **eval: executes** code, **echo: shows** the code

    -   #\|echo: false --\> code won't be [shown]{.underline} when rendering in automated stated ("rerun R"); clicking on play button will load code anyway

-   Packages: Need to rerun pacman when you reopen R document later on

-   After importing data, Check data: check "Environment" tab to see data & cross-check it with dataset

    -   Clicking on play button: to see attribute names

    -   Clicking on table button: to retrieve datatable

-   sf_crs needed for projection of distance-weight matrix, but not spatial clustering (but to be safe, can put crs in)

-   When using Google to search queries, add in context (e.g. how to import shpfile using sf **package** in R, rather than just importing shpfile in R)

-   Housekeeping: Environment tab \> Broom button (so as to cache on memoery)

# Revision on Chapter 5  Geographical Segmentation with Spatially Constrained Clustering Techniques

## Overview

In this hands-on exercise, I gain hands-on experience on how to delineate homogeneous region by using geographically referenced multivariate data. There are two major analysis, namely:
...
In this section, I import Myanmar Township Boundary GIS data and its associated attrbiute table into R environment.
...
-mutate(): to define new functions

-rename(): to rename variables -\> so it is easier to do relational join
...
-   if ggplot(), can only view but won't be saved as output object. Charts will be saved to \_site (html files folder), and can be copy-pasted into document (visual editor mode).

-   g\<- ggplot(): will have output object (can be viewed in "Environment" tab)
...
-   Note sequence of relational join
...
In order to reveal the distribution shown in the choropleth map above are bias to the underlying total number of households at the townships, we will create 2 choropleth maps, one for the total number of households (i.e. TT_HOUSEHOLDS.map) and one for the total number of household with Radio (RADIO.map) by using the code chunk below.
...
[**Can you identify the differences?**]{.underline} The total households and number radio maps show that the areas with more households have more number of radios. However, the TT_Households and Radio_PR show that that some areas which do not have the most households have the highest radio penetration rate (4 areas in dark red on the RADIO_PR map). Thus, we should check the correlation between attributes in the next section.
...
-   **"Fatter" circle: less correlated, vice versa**
...
**-Purpose: to prep data (see shan_ict data table in "Environment" tab), so that can call object later on**
...
**-Purpose: check that shan_ict is left with attributes you need for analysis & also can use township index for clustering dendrogram.**
...
-   **cex** -\> to scale down plot to show township names
...
rect.hclust(hclust_ward, 
            k = 6, 
            border = 2:5) #k for no of clusters determined from optimal cluster graph above, border for colours
```
...
Alternatively, plot each cluster as box plot to see mean, s.d., Q1, Q3 etc.
...
First, we need to convert `shan_sf` into SpatialPolygonsDataFrame. This is because **SKATER function only support *sp* objects such as SpatialPolygonDataFrame. --\> [Need to convert sf data into sp.]{.underline}**
...
### [Ward]{.underline}-like hierarchical clustering: ClustGeo
...
Note that the dissimilarity matrix must be an object of class `dist`, i.e. an object obtained with the function `dist()`. For sample code chunk, refer to [5.7.6 Computing proximity matrix](https://r4gdsa.netlify.app/chap05.html#computing-proximity-matrix)
...
-   NOTE: **clust_geo can also consider distance matrix**
...
-   NOTE: use 'capital letter K' rather than small k --\> both refers to number of clusters determined from Ward dendrogram above. (Source: [Introduction to ClustGeo](https://cran.r-project.org/web/packages/ClustGeo/vignettes/intro_ClustGeo.html))

```{r}
cr <- choicealpha(proxmat, distmat, range.alpha = seq(0, 1, 0.1), K=6, graph = TRUE)
```

With reference to the graphs above, [**alpha = 0.3 (cut off value in graph above)**]{.underline} will be used as shown in the code chunk below.

-   NOTE: **Alpha value ranges from 0 (only attribute homogeneity) \<-\> 1 (only spatial homogeneity), thus try to balance them out.**
